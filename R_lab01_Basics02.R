...
Score1<-read.csv("D:/Assignments/Score2.csv",header=T,sep=",")
...
wb<-loadWorkbook("Book3.xlsx")  # read_excel()
sheet<-readWorksheet(wb,"Sheet1",header=T)

################ Writing Output Files #########################################################################

#### writing into a CSV file
write.csv(Score,"data.csv", row.names=F) # pd.to_csv()
...
for ( i in x){
  if (i == 4){
    #break  # to terminate the looop use break 
    next
  }
print (i)
  }
...
y<-data.frame(A=c(1,2,3,4),B=c(5,6,7,8), s= c('a','b'))
y
square(y)
str(y)
...
apply(data,2,min) #This generates the min values for each numeric attribute
                  # axis = 0 row level , axis = 1 column
apply(data,1,max)
##writing this to a data frame
A<-apply(data[,2:11],2,min)
A<-apply(data[,c(2,4,11)],2,min)
...
setwd("D:/dat1")
# read.csv
system.time(DF1 <- read.csv("loan.csv"))   

#sqldf
require(sqldf)
system.time(SQLDF <- read.csv.sql("loan.csv",dbname=NULL)) 

#fread 
require(data.table)
system.time(DT <- fread("loan.csv")) 
...
require(ff)
system.time(FFDF <- read.csv.ffdf(file="loan.csv",nrows=n))   
